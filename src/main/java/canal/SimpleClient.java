...
public class SimpleClient {
    ...
    public static void main(String[] args) throws Exception {
        CanalConnector connector = CanalConnectors.newSingleConnector(
                new InetSocketAddress("192.168.32.128", 11111), "example", "", "");

        connector.connect();
        connector.subscribe(".*\\..*");
        connector.rollback();
        String aa ="<div id=\"jbrw_textarea\" readonly =\"readonly\" ></div>";
    ...
    private static void printEntries(List<Entry> entries) throws Exception {
        for (Entry entry : entries) {
            ...
            System.out.println("操作信息"+db+"----"+table);
            ...
            for (RowData rowData : rowChange.getRowDatasList()) {
                switch (rowChange.getEventType()) {
                    ...
                    case DELETE:
                        ...
                        printColumns(rowData.getBeforeColumnsList());//修改前数据
                        break;
